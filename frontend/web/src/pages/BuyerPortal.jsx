...
export default function BuyerPortal({ user, onBack, initialTab = 'profile' }) {
  ...
  // CARGAR CARRITO Y WISHLIST DESDE localStorage AL INICIAR
  useEffect(() => {
  ...
  if (showCheckout) return <CheckoutPortal cart={cart} cartTotal={cartTotal} user={user} onBack={() => setShowCheckout(false)} onSuccess={() => { setCart([]); setShowCheckout(false); }} />;
